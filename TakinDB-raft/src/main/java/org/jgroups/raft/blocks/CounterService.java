...
public class CounterService implements StateMachine, RAFT.RoleChange {
    ...
    protected enum Command {
        create, delete, get, set, compareAndSet, incrementAndGet, decrementAndGet, addAndGet
    }
    ...
    public String printCounters() {
        return counters.entrySet().stream().collect(StringBuilder::new, (sb, entry) -> sb.append(entry.getKey()).append(" = ").append(entry.getValue()).append("\n"), (l, r) -> {
        }).toString();
    }
    ...
    public void dumpLog() {
        raft.logEntries((entry, index) -> {
            ...
            if (entry.internal()) {
                try {
                    InternalCommand cmd = (InternalCommand) Util.streamableFromByteBuffer(InternalCommand.class, entry.command(), entry.offset(), entry.length());
