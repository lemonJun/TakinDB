...
public class JUnitXMLReporter implements ITestListener, IConfigurationListener2 {
    ...
    protected void setupStreams(ITestResult result, boolean printMethodName) {
        ...
        if (!dir.exists())
            dir.mkdirs();
        File _tests = new File(dir, TESTS), _stdout = new File(dir, STDOUT),
    ...
    protected void addTest(Class<?> clazz, ITestResult result) {
        try {
            TestCase test_case = new TestCase(result.getStatus(), clazz.getName(), getMethodName(result), result.getStartMillis(), result.getEndMillis());
    ...
    protected static void generateReport(Writer out, String classname, List<TestCase> results, Reader stdout, Reader stderr) throws IOException {
        ...
        try {
            ...
            out.write("\n<testsuite " + "name=\"" + classname + "\" " + "tests=\"" + results.size() + "\" " + "failures=\"" + num_failures + "\" " + "errors=\"" + num_errors + "\" " + "skips=\"" + num_skips + "\" " + "time=\"" + (total_time / 1000.0) + "\">");
    ...
    public static int getErrors(Collection<TestCase> results) {
        int retval = 0;
        for (TestCase result : results) {
            if (result != null && result.status != ITestResult.SUCCESS && result.status != ITestResult.SUCCESS_PERCENTAGE_FAILURE && result.status != ITestResult.FAILURE)
                retval++;
        }
        return retval;
    }
    ...
    public static class TestCase implements Streamable {
        ...
        public String toString() {
            StringBuilder sb = new StringBuilder();
            sb.append(statusToString(status)).append(" ").append(classname).append(".").append(name).append(" in ").append(getTime()).append(" ms");
