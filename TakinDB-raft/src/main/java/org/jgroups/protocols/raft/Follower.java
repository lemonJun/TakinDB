...
public class Follower extends RaftImpl {
    ...
    protected void handleInstallSnapshotRequest(Message msg, int term, Address leader, int last_included_index, int last_included_term) {
        ...
        try {
            ...
            raft.getLog().debug("%s: applied snapshot (%s) from %s; last_appended=%d, commit_index=%d", raft.local_addr, Util.printBytes(msg.getLength()), msg.src(), raft.lastAppended(), raft.commitIndex());
