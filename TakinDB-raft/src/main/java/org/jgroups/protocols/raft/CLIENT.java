...
public class CLIENT extends Protocol implements Runnable {
    ...
    public enum RequestType {
        set_req, add_server, remove_server, type, rsp
    }
    ...
    @Property(name = "bind_addr", description = "The bind address which should be used by the server socket. The following special values " + "are also recognized: GLOBAL, SITE_LOCAL, LINK_LOCAL, NON_LOOPBACK, match-interface, match-host, match-address", systemProperty = { Global.BIND_ADDR }, writable = false)
    ...
    public void start() throws Exception {
        ...
        thread_pool = new ThreadPoolExecutor(min_threads, max_threads, idle_time, TimeUnit.MILLISECONDS, new SynchronousQueue<>(), getThreadFactory(), new ThreadPoolExecutor.DiscardPolicy());
    ...
    protected class RequestHandler implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                switch (type) {
                    case set_req:
                        settable.setAsync(buffer, 0, buffer.length).whenComplete(new CompletionHandler(client_sock, in, out));
                        break;
                    case add_server:
