...
public class AppendEntriesTest {
    ...
    static {
        try {
            handleAppendEntriesRequest = RaftImpl.class.getDeclaredMethod("handleAppendEntriesRequest", byte[].class, int.class, int.class, Address.class, int.class, int.class, int.class, int.class, boolean.class);
    ...
    protected final void assertSame(ReplicatedStateMachine<Integer, Integer>... rsms) {
        ...
        for (int j = 1; j < rsms.length; j++) {
            ReplicatedStateMachine<Integer, Integer> rsm = rsms[j];
            assert rsm.equals(first) : String.format("commit-table of A: %s", ((RAFT) a.getProtocolStack().findProtocol(RAFT.class)).dumpCommitTable());
        }
    }
    ...
    protected void assertCommitIndex(long timeout, long interval, int expected_commit, int expected_applied, JChannel... channels) {
        ...
        for (JChannel ch : channels) {
            RAFT raft = raft(ch);
            System.out.printf("%s: last-applied=%d, commit-index=%d\n", ch.getAddress(), raft.lastAppended(), raft.commitIndex());
            assert raft.commitIndex() == expected_commit && raft.lastAppended() == expected_applied : String.format("%s: last-applied=%d, commit-index=%d", ch.getAddress(), raft.lastAppended(), raft.commitIndex());
        }
    }
    ...
    protected AppendResult append(RaftImpl impl, byte[] data, Address leader, int prev_log_index, int prev_log_term, int entry_term, int leader_commit) throws Exception {
        return (AppendResult) handleAppendEntriesRequest.invoke(impl, data, 0, data.length, leader, prev_log_index, prev_log_term, entry_term, leader_commit, false);
    }
