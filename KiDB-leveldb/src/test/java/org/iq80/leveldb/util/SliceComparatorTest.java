...
public class SliceComparatorTest {
    ...
    public void testSliceComparison() {
        assertTrue(SLICE_COMPARATOR.compare(Slices.copiedBuffer("beer/ipa", Charsets.UTF_8), Slices.copiedBuffer("beer/ale", Charsets.UTF_8)) > 0);

        assertTrue(SLICE_COMPARATOR.compare(Slices.wrappedBuffer(new byte[] { (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF }), Slices.wrappedBuffer(new byte[] { (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00 })) > 0);

        assertTrue(SLICE_COMPARATOR.compare(Slices.wrappedBuffer(new byte[] { (byte) 0xFF }), Slices.wrappedBuffer(new byte[] { (byte) 0x00 })) > 0);

        assertAllEqual(Slices.copiedBuffer("abcdefghijklmnopqrstuvwxyz", Charsets.UTF_8), Slices.copiedBuffer("abcdefghijklmnopqrstuvwxyz", Charsets.UTF_8));
    }
