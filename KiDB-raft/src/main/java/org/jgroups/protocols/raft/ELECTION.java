...
public class ELECTION extends Protocol {
    ...
    public void init() throws Exception {
        ...
        if (election_min_interval >= election_max_interval)
            throw new Exception("election_min_interval (" + election_min_interval + ") needs to be smaller than " + "election_max_interval (" + election_max_interval + ")");
    ...
    protected void handleVoteRequest(Address sender, int term, int last_log_term, int last_log_index) {
        ...
        if (log.isTraceEnabled())
            log.trace("%s: received VoteRequest from %s: term=%d, my term=%d, last_log_term=%d, last_log_index=%d", local_addr, sender, term, raft.currentTerm(), last_log_term, last_log_index);
    ...
    protected void sendHeartbeat(int term, Address leader) {
        Message req = new Message(null).putHeader(id, new HeartbeatRequest(term, leader)).setFlag(Message.Flag.OOB, Message.Flag.INTERNAL, Message.Flag.NO_RELIABILITY, Message.Flag.NO_FC).setTransientFlag(Message.TransientFlag.DONT_LOOPBACK);
    ...
    protected void sendVoteRequest(int term) {
        ...
        Message vote_req = new Message(null).putHeader(id, req).setFlag(Message.Flag.OOB, Message.Flag.INTERNAL, Message.Flag.NO_RELIABILITY, Message.Flag.NO_FC).setTransientFlag(Message.TransientFlag.DONT_LOOPBACK);
    ...
    protected void sendVoteResponse(Address dest, int term) {
        ...
        Message vote_rsp = new Message(dest).putHeader(id, rsp).setFlag(Message.Flag.OOB, Message.Flag.INTERNAL, Message.Flag.NO_RELIABILITY, Message.Flag.NO_FC);
