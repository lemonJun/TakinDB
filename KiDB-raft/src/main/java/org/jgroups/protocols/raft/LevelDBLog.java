...
/**
 * Implementation of ${link #Log} with leveldb
 * 与mapdb 两者选一个即可
 * @author Ugo Landini
 */
public class LevelDBLog implements Log {
    ...
    private LogEntry getLogEntry(int index) {
        ...
        try {
            if (entryBytes != null)
                entry = (LogEntry) Util.streamableFromByteBuffer(LogEntry.class, entryBytes);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return entry;
    ...
    private void readMetadataFromLog() throws Exception {
        ...
        log.debug("read metadata from log: firstAppended=%d, lastAppended=%d, currentTerm=%d, commitIndex=%d, votedFor=%s", firstAppended, lastAppended, currentTerm, commitIndex, votedFor);
    }
